# Vencord

![](https://img.shields.io/github/package-json/v/ragecord-dev/ragecord?style=for-the-badge&logo=github&logoColor=d3869b&label=&color=1d2021&labelColor=282828)
[![Codeberg Mirror](https://img.shields.io/static/v1?style=for-the-badge&label=Codeberg%20Mirror&message=codeberg.org/Vee/cord&color=2185D0&logo=data:image/png;base64,)](https://codeberg.org/Vee/cord)

The cutest Discord client mod

![](https://github.com/user-attachments/assets/3fac98c0-c411-4d2a-97a3-13b7da8687a2)

## Features

-   Easy to install
-   [100+ built in plugins](https://vencord.dev/plugins)
-   Fairly lightweight despite the many inbuilt plugins
-   Excellent Browser Support: Run RageCord in your Browser via extension or UserScript
-   Works on any Discord branch: Stable, Canary or PTB all work
-   Custom CSS and Themes: Inbuilt css editor with support to import any css files (including BetterDiscord themes)
-   Privacy friendly: blocks Discord analytics & crash reporting out of the box and has no telemetry
-   Maintained very actively, broken plugins are usually fixed within 12 hours
-   Settings sync: Keep your plugins and their settings synchronised between devices / apps (optional)


## Installing / Uninstalling

Visit https://vencord.dev/download

## Join our Support/Community Server

https://discord.gg/UVy7Rmdb

## Sponsors

|     **Thanks a lot to all Vencord [sponsors](https://github.com/sponsors/Vendicated)!!**     |
| :------------------------------------------------------------------------------------------: |
|   [![](https://meow.vendicated.dev/sponsors.png)](https://github.com/sponsors/Vendicated)    |
| *generated using [github-sponsor-graph](https://github.com/Vendicated/github-sponsor-graph)* |


## Star History

<a href="https://star-history.com/#ragecord-dev/RageCord&Timeline">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=ragecord-dev/RageCord&type=Timeline&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=ragecord-dev/RageCord&type=Timeline" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Vendicated/Vencord&type=Timeline" />
  </picture>
</a>

## Disclaimer

**RageCord is an independent project and is not affiliated with, endorsed by, or sponsored by Discord Inc.**  
“Discord” is a registered trademark of Discord Inc. and is used here strictly for descriptive purposes.

<details>
<summary>⚠️ Important: Use at your own risk</summary>

Using client modifications like RageCord **violates Discord's Terms of Service**.

While Discord has historically not enforced bans for using client mods like RageCord, this could change at any time. Use is **at your own risk**, especially if your account is important to you.

RageCord does **not include malicious or abusive features**, and all official plugins are designed with safety in mind. However, any unofficial or third-party plugins you install may put your account at risk.

We do **not recommend** using RageCord on accounts that are:
- Moderating communities
- Holding sensitive data
- Business or partnership-related

You are solely responsible for any consequences resulting from the use of RageCord.

</details>
